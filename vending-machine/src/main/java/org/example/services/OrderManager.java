    ...
    try {
      if (change > 0) {
        Map<Currency, Long> changeCurrencyMap =
            CurrencyManager.getChangeCurrencyMap(change, _currencyManager.getAvailableCash());
      ...
      // Not passing order here directly to handle multiple item dispatches if placed in a single
